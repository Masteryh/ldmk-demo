...
def get_distance_from_ldmks(ldmk_tail, landmk_head, X=1, Y=1):
    ldmk_dis = np.linalg.norm(
        [landmk_head['x'] * X, landmk_head['y'] * Y] - np.array([ldmk_tail['x'] * X, ldmk_tail['y'] * Y]))
    return ldmk_dis
...
def main():
    ...
    parser.add_argument('--flip', '-f', type=bool, default=False,
                        help='Set to any value to flip resulting output (selfie view)')
    ...
    parser.add_argument('--record', '-r', type=bool, default=False,
                        help='Set to save a timestamped AVI in current directory')
